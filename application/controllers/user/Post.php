...
class Post extends MY_Controller {
	...
	public function index()
	{
		...
		$data['page_menu']='post';
		$data['list_post']=$this->Post_M->all(['post_user_id' => $this->user_id],'desc');
		$this->getHeaderUser($data);
		$this->load->view('user/pages/post/index.php',$data);
		$this->getFooter();
	}
	...
	public function add()
	{
		...
		if ($this->input->post()) {
			...
			redirect(base_url('user/post/add/'),'location');
		...
		$data['page_menu']='post';
		$this->getHeaderUser($data);
		$this->load->view('user/pages/post/add.php',$data);
		$this->getFooter();
	}
	...
	public function edit($id)
	{
		...
		if ($this->input->post()) {
			...
			$this->session->set_flashdata('reponse',$status);
			redirect(base_url('user/post/edit/'.$id),'location');
		...
		$data['page_menu']='post';
		$this->getHeaderUser($data);
		$this->load->view('user/pages/post/edit.php',$data);
		$this->getFooter();
	}
